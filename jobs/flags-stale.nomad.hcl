...
variable "unleash_url" {
  type        = string
  default     = "http://127.0.0.1:26065"
  description = "Unleash base URL (loopback, discovered 2026-09-19; re-point to the public hostname after the TF cutover)."
}
variable "scan_repo" {
  type        = string
  default     = ""
  ...
  group "stale" {
    ...
    task "stale" {
      ...
      config {
        ...
        args = [
          "python3", "/app/unleash-stale-tickets.py",
          "--unleash", "${UNLEASH_URL}",
        ]
      }
