...
job "flags-listener" {
  ...
  group "listener" {
    ...
    task "listener" {
      ...
      config {
        # Host netns: the scripts bind 127.0.0.1, which must be the HOST
        # loopback (tunnel-only posture; nothing on the public interface).
        network_mode = "host"
        ...
        ports = ["http"]
        args = [
          "python3", "/app/flags-listener.py", "--serve",
          "--port", "${NOMAD_PORT_http}",
        ]
      }

      env {
        FLAGS_WEBHOOK_SECRET = var.webhook_secret
      ...
      service {
        ...
        tags = [
          "traefik.enable=true",
          "traefik.http.routers.flags-listener.rule=Host(`flags-listener.pkubelka.cz`)",
          "traefik.http.routers.flags-listener.entrypoints=web",
        ]
      }
    }
  }
