...
- Cloudflare DNS records for the Coolify dashboard, SSH Tunnel hostname, adopted app hostnames (`fabric`, `omniroute`, `omni`, plus plan-only `registry`), and optional application wildcard.
- Cloudflare Tunnel ingress from `ssh.<domain>` to `localhost:22`, dashboard to `localhost:8000`, and app hostnames to `localhost:80`.
