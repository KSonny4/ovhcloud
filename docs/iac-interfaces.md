...
This document defines the small interfaces between provider authorization, Terraform, guest bootstrap, Nomad, Cloudflare Access, and OpenBao. The implementations may change; callers should depend on these contracts rather than on ad-hoc shell variables or browser state.
...
One invocation executes bootstrap -> Nomad -> Tunnel/Access -> R2 backup
...
- Inputs: `PROVISION_HOST`, `PROVISION_ZONE` (Cloudflare zone),
  ...
  `NOMAD_VERSION` (pinned; default `2.0.6`) and `R2_ENDPOINT` (defaults
  to the account endpoint). ACL bootstrap runs inside the nomad stage
  and escrows automatically — no first-admin password input exists.
  The UI hostname is always `nomad.${PROVISION_ZONE}` (single
  domain contract with Terraform; a `PROVISION_UI_HOST` override
  does not exist and is refused fail-closed). It is used consistently
  for Nomad FQDN, Tunnel ingress/DNS, and every HTTP verification.
...
- Retrieves from OpenBao by name only: `PROVISION_SSH_PUBLIC_KEY.value`,
  per-target tunnel entry `EDGE_TUNNEL_<NAME>.{tunnel_id,tunnel_token}`
  (derived from `TUNNEL_NAME`; `nomad-admin` is refused). Tunnel-entry
  roles are disjoint by design: `EDGE_TUNNEL_SECRET.tunnel_secret`
  feeds ONLY the preserved Terraform tunnel config via the loader;
  `EDGE_TUNNEL_TOKEN.tunnel_token` is the preserved connector's cold
  ...
  `EDGE_ACCESS_SERVICE_TOKEN.{client_id,client_secret,token_id}`,
...
- Copies the stage scripts (bootstrap, provision, tunnel/access, schedule +
  ...
  hello-world, origin leader endpoint, edge wiring + UI HTTP 200, timer
...
3. **access** — manage a human UI policy for `ksonny4@gmail.com`, a narrowly scoped machine service token, and the identity-provider configuration needed by optional human use;
...
5. **nomad** — install a pinned Nomad release, bind loopback-only, ACL-bootstrap with automatic escrow, and expose health state;
...
| Application secrets (derived, per app) | OpenBao per-app entries (generate-if-absent + escrow; reuse otherwise) | manifests mark them escrow-recoverable (`env_escrowed`); restore re-injects via `fetch-app-secrets.sh` with no human relay |
...
- `nomad_ready`: release, UI hostname, ACL bootstrap + escrow, server members, and leader endpoint pass;
...
A human may use the UI after deployment, but no provisioning or machine verification step may depend on that browser session.
