...
1. RESOLVED 2026-09-14: production state import + authorized live apply done —
   ...
   escrow record remains). Credential files are generated by
   `scripts/load-tfvars-from-openbao.sh`, never hand-populated.
2. PARTLY RESOLVED 2026-09-14: scoped R2 S3 credential minted via dashboard
   and escrowed (`COOLIFY_R2` v1); live probe + nightly schedule + RESTORE_OK
