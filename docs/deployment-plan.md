...
| `docs/04-cloudflare.md:13-64`, `docs/09-docker-registry.md:24-30` | Cloudflare is the public edge; API clients must not be forced through interactive Access | DNS, tunnel, private R2 and admin Access are represented in IaC; application API authentication stays application-owned | Ready; API auth is application-owned |
| `docs/05-backup-recovery.md` | Recovery depends on R2, Nomad bootstrap material, application keys and tested restores | Secret inventory, encrypted-state requirement, backup/restore gates and rollback are now explicit | Workload proofs DONE 2026-09-14 (`RESTORE_OK`, byte-identical, `CONNECT_OK`); Nomad snapshot drill is the M5 gate; see `docs/08-iac-redesign-evidence.md` |
...
- Cloudflare DNS CNAMEs for the Nomad UI and SSH hostname point at the Tunnel (`<tunnel-id>.cfargotunnel.com`), not at origin A records; application hostnames are likewise Tunnel CNAMEs with no Access app; the application wildcard is opt-in and disabled by default.
...
The authorized operator must provide a Cloudflare-managed zone and decide whether the wildcard is enabled. The plan is reviewed with values supplied via the two-step loader form (`loader_out="$(...)" || exit 2`, then `eval "$loader_out"`; env-only, no tfvars file is ever written).
...
| OmniRoute secrets (`OMNIROUTE`) | retired 2026-09-19 | entry no longer consumed by any script; delete after cutover verifies | — |
