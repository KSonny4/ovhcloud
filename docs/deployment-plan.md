...
| Cloudflare Tunnel secret | platform operator | OpenBao per-target `secret/projects/ovhcloud/EDGE_TUNNEL_<NAME>` (`tunnel_secret`) via `ensure-tunnel.sh` (create + escrow, never the preserved `nomad-admin` singleton) | rotate tunnel and Access policy after exposure |
...
| Nomad bootstrap material | Nomad owner | OpenBao `secret/projects/ovhcloud/NOMAD_BOOTSTRAP` (`acl_token`, `acl_accessor`, `gossip_key`): provisioner ACL-bootstraps over SSH + escrows (memory-only, fail closed) | restore test must re-administer from escrow |
...
| Cloudflare machine service token | Terraform-generated, escrowed in OpenBao | `secret/projects/ovhcloud/EDGE_ACCESS_SERVICE_TOKEN` (`client_id`, `client_secret`) | noninteractive verification must pass without browser login |
| Docker registry htpasswd + `REGISTRY_HTTP_SECRET` | registry owner | OpenBao `secret/projects/ovhcloud/REGISTRY` (`htpasswd`, `http_secret`): generated off-host, relayed into the Nomad job file mount/env memory-only, never committed | rotate htpasswd + redeploy; verify with authenticated `docker login` |
