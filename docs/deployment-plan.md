...
- Cloudflare Access: human OTP/email policy retained plus a scoped machine service token (`non_identity`) escrowed to OpenBao via `vault_kv_secret_v2.access_service_token`; no provisioning step depends on browser login.
