...
TF_VAR_cloudflare_api_token="$(bao kv get -field=ADMIN_CLOUDFLARE secret/projects/nomad/ADMIN_CLOUDFLARE)"
TF_VAR_cloudflare_account_id="$CLOUDFLARE_ACCOUNT_ID"
TF_VAR_cloudflare_zone_id="$CLOUDFLARE_ZONE_ID"
TF_VAR_service_token_id="$(bao kv get -field=token_id secret/projects/nomad/EDGE_ACCESS_SERVICE_TOKEN)"
...
for v in AWS_ACCESS_KEY_ID AWS_SECRET_ACCESS_KEY; do
  [ -n "${!v}" ] || { echo "OpenBao BACKUP_R2 escrow missing for ${v} (fail closed)." >&2; exit 2; }
done
