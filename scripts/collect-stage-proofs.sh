...
# only (never tokens, keys, or values) to docs/live-proofs-stages.json:
# preserved-safety, bootstrap, coolify, tunnel-access, r2-backup, restore,
...
out="$repo_root/docs/live-proofs-stages.json"
stamp=''
while [ "$#" -gt 0 ]; do
  case "$1" in
...
ev_ssh_key="${EVIDENCE_SSH_KEY:-$HOME/.ssh/ovh_coolify_ed25519}"
...
# Stage 3: coolify (healthy platform containers + origin login route).
t0="$(uts)"
healthy="$($ssh 'docker ps --format "{{.Names}} {{.Status}}" 2>/dev/null | grep -cE "Up|healthy" || true')"
origin_code="$($ssh 'curl -s -o /dev/null -w "%{http_code}" --max-time 15 http://127.0.0.1:8000/login 2>/dev/null || true')"
if [ "${healthy:-0}" -ge 6 ] && [ "$origin_code" = '200' ]; then
  record coolify "$t0" 1 "healthy_containers=${healthy}; origin_login=${origin_code}"
else
  record coolify "$t0" 0 "healthy_containers=${healthy}; origin_login=${origin_code}"
fi
...
# Stage 4: tunnel-access (machine 200 on dashboard, gated code on ssh route).
t0="$(uts)"
dash_code="$(curl -s -o /dev/null -w '%{http_code}' --max-time 30 -H "CF-Access-Client-Id: ${svc_id}" -H "CF-Access-Client-Secret: ${svc_secret}" 'https://coolify.pkubelka.cz/login' 2>/dev/null || true)"
ssh_code="$(curl -s -o /dev/null -w '%{http_code}' --max-time 30 'https://ssh.pkubelka.cz/' 2>/dev/null || true)"
if [ "$dash_code" = '200' ] && { [ "$ssh_code" = '301' ] || [ "$ssh_code" = '302' ] || [ "$ssh_code" = '401' ] || [ "$ssh_code" = '403' ]; }; then
  record tunnel-access "$t0" 1 "dashboard_login=${dash_code}; ssh_route_gated=${ssh_code}"
else
  record tunnel-access "$t0" 0 "dashboard_login=${dash_code}; ssh_route=${ssh_code}"
fi

# R2 handles (memory-only env, same pattern as the collectors).
r2_ak="$(bao kv get -field=access_key_id secret/projects/ovhcloud/COOLIFY_R2 2>/dev/null || true)"
r2_sk="$(bao kv get -field=secret_access_key secret/projects/ovhcloud/COOLIFY_R2 2>/dev/null || true)"
...
if [ -z "$stamp" ]; then
  stamp="$(aws --endpoint-url "$r2_ep" s3 ls 's3://ovh-coolify-backups/app-manifests/' 2>/dev/null | awk '{print $4}' | grep -oE '[0-9]{8}T[0-9]{6}Z' | sort | tail -n1 || true)"
fi
[ -n "$stamp" ] || { echo 'no app manifest stamp found.' >&2; exit 2; }

# Stage 5: r2-backup (reseed stamps exist for the current plane).
t0="$(uts)"
dump_key="$(aws --endpoint-url "$r2_ep" s3 ls 's3://ovh-coolify-backups/' 2>/dev/null | awk '{print $4}' | grep '^coolify-db-' | sort | tail -n1 || true)"
manifest_key="${stamp}.json"
if [ -n "$dump_key" ] && aws --endpoint-url "$r2_ep" s3api head-object --bucket ovh-coolify-backups --key "app-manifests/${manifest_key}" >/dev/null 2>&1; then
  record r2-backup "$t0" 1 "instance_dump=${dump_key}; app_manifest=${manifest_key}"
else
  record r2-backup "$t0" 0 "instance_dump=${dump_key}; app_manifest=${manifest_key}"
fi

# Stages 6-7: restore + rollback probes (repo code staged to /tmp, cleaned).
scp -i "$ev_ssh_key" -o BatchMode=yes -o ConnectTimeout=15 "$repo_root/scripts/rollback-coolify-backup.sh" "$repo_root/scripts/rollback-app-workloads.sh" "${ev_ssh_host}:/tmp/" >/dev/null
t0="$(uts)"
restore_out="$($ssh 'sudo /root/coolify-backup/fetch-r2-env.sh -- bash /tmp/rollback-coolify-backup.sh 2>&1' | grep -aE 'RESTORE_OK|FAILED' | head -n2 || true)"
...
t0="$(uts)"
rollback_log="$($ssh "sudo /root/coolify-backup/fetch-r2-env.sh -- bash /tmp/rollback-app-workloads.sh --stamp $stamp 2>&1" || true)"
...
$ssh 'rm -f /tmp/rollback-coolify-backup.sh /tmp/rollback-app-workloads.sh' >/dev/null 2>&1 || true
...
echo "stage proofs collected -> docs/live-proofs-stages.json (failures: ${fail_n})."
[ "$fail_n" -eq 0 ]
