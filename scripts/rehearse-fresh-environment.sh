...
[{"Name": "/runtime-app", "Config": {"Image": "python:3.12-alpine", "Env": ["APP_MODE=proof", "DB_PASSWORD=s3cret"], "Labels": {"proof": "runtime"}, "Cmd": ["python3", "-m", "http.server", "8080"], "Entrypoint": null, "WorkingDir": "/srv/www", "User": "65534", "Healthcheck": {"Test": ["CMD-SHELL", "wget -q -O /dev/null http://localhost:8080/"], "Interval": 30000000000, "Timeout": 5000000000, "Retries": 3}}, "HostConfig": {"PortBindings": {"8080/tcp": [{"HostIp": "", "HostPort": "18081"}]}, "RestartPolicy": {"Name": "unless-stopped", "MaximumRetryCount": 0}}, "Mounts": [{"Type": "volume", "Source": "/var/lib/docker/volumes/runtime-www/_data", "Destination": "/srv/www", "Mode": "rw"}], "NetworkSettings": {"Networks": {"bridge": {}}}}]
INSPECT_EOF
...
for want in '"ports": ["18081:8080/tcp"]' '"DB_PASSWORD": "REDACTED"' '"APP_MODE": "proof"' '"source": "/var/lib/docker/volumes/runtime-www/_data"' '"target": "/srv/www"' '"cmd": ["python3", "-m", "http.server", "8080"]' '"workdir": "/srv/www"' '"user": "65534"' '"restart": "unless-stopped"' '"CMD-SHELL"'; do
